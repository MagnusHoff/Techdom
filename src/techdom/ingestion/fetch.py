...
def get_prospect_or_scrape(
    ...
    *,
    ...
    Hovedinngang for UI-knappen "Hent data":
      ...
      Lokalt speil (kun debug): eventuell kopi i data/cache/prospekt.
    """
    dbg: Dict[str, Any] = {
...
def fetch_prospectus_from_megler_url(
    megler_url: str,
    *,
    save_dir: str = "data/cache/prospekt",
) -> Tuple[bytes | None, str | None, dict]:
    dbg: Dict[str, Any] = {
        "step": "start",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Bruk: python -m techdom.ingestion.fetch <FINN_URL> [--verify-remote]")
        sys.exit(1)
    url = sys.argv[1]
